    ...
    N: Negotiation,
{
    ...
    ///
    /// On success, returns the response PDU, negotiated options, and the called AE title.
    /// On failure, returns a rejection/abort PDU and an error.
    #[allow(clippy::result_large_err)]
    pub fn process_association_rq(
        &self,
        msg: Pdu,
    ) -> std::result::Result<(Pdu, NegotiatedOptions, String), (Pdu, Error)> {
        self.process_a_association_rq(msg)
    }
...
where
    ...
    N: Negotiation,
{
    /// Negotiate an association with the given TCP stream and a pre-read association request PDU.
    ///
    /// This is intended for multi-AE SCP deployments that route by called AE title before
    /// completing negotiation with per-AE capabilities.
    pub async fn establish_async_with_rq<S>(
        &self,
        mut socket: S,
        pdu: Pdu,
        read_buffer: BytesMut,
    ) -> Result<AsyncServerAssociation<S>>
    where
    ...
    pub async fn establish_async(
        ...
        let task = async {
            ...
            self.establish_async_with_rq(socket, pdu, read_buffer).await
        };
        super::timeout(read_timeout, task).await
    }
