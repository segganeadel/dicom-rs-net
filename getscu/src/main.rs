...
/// DICOM Study Root C-GET SCU
#[derive(Debug, Parser)]
#[command(version)]
struct App {
    /// Remote SCP address (example: `PACS@127.0.0.1:11112`)
    addr: String,
    /// Path to a pre-built C-GET identifier dataset
    #[arg(long = "identifier")]
    identifier_file: Option<PathBuf>,
    /// SOP Instance UID for image-level retrieve
    #[arg(long = "sop-instance-uid")]
    sop_instance_uid: Option<String>,
    /// Study Instance UID for study-level retrieve
    #[arg(long = "study-instance-uid")]
    study_instance_uid: Option<String>,
    /// Calling Application Entity title
    #[arg(long = "calling-ae-title", default_value = "GETSCU")]
...
async fn run(app: App) -> Result<(), Box<dyn std::error::Error>> {
    ...
    scu.add_scu_capability(TransferCapability::query_retrieve_get_scu(STUDY_ROOT_GET));

    let counts = scu
        .get_instances(&conn, &app.addr, &identifier)
        .await?;
