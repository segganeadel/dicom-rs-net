...
**Phase 6b (Interop & API maturity):** complete — optional interop CI; API **0.2** (`DeviceBuilder` removed, `qr` tier frozen).
...
| Phase 6b (done) | Phase 1 (done) | Interop CI, API stabilization |
| Phase 7 | Phase 2 | Patient Root Q/R |
| — | Phase 3 | `pacs-web`, admin UI, OSS viewers |
...
- [x] API stabilization **0.2** after **pacs** Phase 1 validation (`docs/STABILITY.md`: remove `DeviceBuilder`, freeze `qr` tier)
